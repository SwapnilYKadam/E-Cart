...
const Cart = () => {
  ...
  const handleCheckout = () => {
    ...
    const addStripe = async () => {
      ...
      const stripePromise = loadStripe("pk_test_51JYS4lSI7Mw1fgd2NZwHQynRq2HT8hxor6ZB3MlQkEPLSmSgCWwhx0HeXVBXKnFFj9KhGGZ3G3LqTk6ukp7r2ZS0008sJAiejl");
        ...
        <div className="cart">
          <div className="cart_items">
            {user.cartItems.map((product, i) => (
              removeProductToCart.loading ? <Spinner /> : < div key={i} className="cart_item" >
                <img
                ...
                <button className="btn" onClick={() => { removeFromCart(product._id, product.price) }}>Remove</button>
              </div>
            ))}
          </div>
          <div className="cart_total">
            ...
            <hr />
            <button id="checkout" className="btn checkout-btn" onClick={handleCheckout}>Checkout</button>
          </div>
        </div > : <div className="noproduct"><p className="message">Your cart is empty.</p></div>
  );
};
